...
if {$opt(reset)} {
  open_project -reset vc709_prj
} else {
  open_project vc709_prj
}
set_top vc709
add_files firmware/vc709.cpp -cflags "-std=c++0x"
add_files -tb vc709_test.cpp -cflags "-std=c++0x"
...
if {$opt(cosim)} {
  ...
  add_files -tb vc709_test.cpp -cflags "-std=c++0x -DRTL_SIM"
  set time_start [clock clicks -milliseconds]
  cosim_design -trace_level all
  set time_end [clock clicks -milliseconds]
  puts "INFO:"
  puts [read [open vc709_prj/solution1/sim/report/vc709_cosim.rpt r]]
...
if {$opt(vsynth)} {
  ...
  if {[file exist vc709_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
